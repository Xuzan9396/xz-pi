...
test("a failed child settles immediately without pausing or retrying", async () => {
  const manager = new TaskManager(); let calls = 0;
  const records = await manager.run([plan("failure")], 1, async () => {
    calls++;
    return { status: "failed", output: "partial", error: "provider failed" };
  });
  assert.equal(calls, 1);
  assert.equal(records[0]?.status, "failed");
  assert.equal(records[0]?.output, "partial");
