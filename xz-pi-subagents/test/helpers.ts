...
export function plan(name = "one", mode: "read" | "write" = "read"): LaunchPlan {
  return { task: { name, task: name, mode }, resources, context: "", tools: ["read"], skillPaths: [], timeoutMs: 10_000 };
}
export function record(name = "one"): TaskRecord {
  return { id: name, name, task: name, mode: "read", exclusive: false, model: "fixture/model", status: "running", activity: "", transcript: "", output: "", tokens: 0 };
}
