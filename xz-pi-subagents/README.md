...
- 子任务没有自动超时，会一直运行到完成、失败、明确取消或会话关闭。
- 失败任务立即进入 `failed` 终态，不等待快捷键处理、不自动重试，也不会阻塞批次；其他任务继续运行。
- 所有任务进入完成、失败或取消状态后才返回，结果保持输入顺序。某个任务失败不会丢弃其他任务的结果；main 可根据错误决定是否另行委派更小、更准确的任务。
...
| 模式 | 默认工具 | 调度 |
| --- | --- | --- |
| `mode: "read"`（默认） | main 当前启用的 `read/bash/grep/find/ls` 的交集 | 默认并行 |
...
- 每个任务在系统临时目录生成独立的 `xz-pi-subagent-*` 私有目录，写入 `output.md`、`events.jsonl`、`result.json`、`stderr.log`，以及启动配置和职责提示词。
