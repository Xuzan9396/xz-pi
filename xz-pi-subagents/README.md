...
| 暂停任务详情 | `c` | 启动一个全新子 Agent，基于当前工作区和上一轮错误摘要继续原任务 |
...
- 子任务没有自动超时，会一直运行到完成、失败后由用户处理、明确取消或会话关闭。
- TUI 中失败任务进入暂停状态：详情按 `c` 会启动全新 Agent，复用原任务配置和当前工作区，并附带上一轮错误摘要；它不恢复上一 Agent 的对话上下文。
- main 会等待暂停任务被继续成功或明确取消；JSON/print 等非交互模式无法接收快捷键，失败会直接结束。
- 所有任务进入完成、失败或取消状态后才返回，结果保持输入顺序。某个任务失败不会丢弃其他任务的结果。
...
- 每个任务在系统临时目录生成独立的 `xz-pi-subagent-*` 私有目录；每次新 Agent 尝试分别写入 `attempt-N/output.md`、`events.jsonl`、`result.json`、`stderr.log`，以及启动配置和职责提示词。
...
测试包含：两种模式默认并发、显式独占、批量等待、失败暂停与全新 Agent 重试、无自动超时、取消竞态、TERM 忽略后的强制回收、JSON/UTF-8 分帧、面板隐藏与 `/agent_show` 重开、取消后自动隐藏与日志保留、UI 导航、宽度限制、Vim 兼容，以及真实 Pi CLI 配合本地模拟模型的集成测试（两个 write 模式子进程调用搜索 fixture，验证它们实际重叠运行）。集成测试不使用真实 API key，也不消耗模型费用。
