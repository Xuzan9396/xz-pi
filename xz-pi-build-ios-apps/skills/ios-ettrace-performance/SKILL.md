...
Run `ettrace` interactively in a foreground terminal or a visible `tmux` pane and answer its prompts directly. Do not launch it through a non-interactive shell: without a TTY, the runner can exit without a useful trace.
