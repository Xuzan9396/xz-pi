...
main 通过一个 `xz_subagents_run` 工具分派 2–8 个独立子任务，并等待全部结果；只有一个任务时由 main 直接执行。子任务没有自动超时，失败后立即结束且不自动重试，不会阻塞其他任务。输入框下方自动显示任务进度；空输入框按 `↓` 或执行 `/agent_show` 打开/重开、`Enter` 查看实时详情、两次 `x` 只取消选中的子任务。read 模式默认提供 `read + bash` 并要求不修改状态，但不是系统级沙箱。任务默认并行，只有 `exclusive: true` 才独占执行。复用 Pi skills 和 MCP 扩展配置，不创建隔离工作区，也不提供后台守护、记忆或复杂编排。
